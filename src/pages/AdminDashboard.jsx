...
// Change these to whatever you want. They never go to Supabase Auth.
...
export default function AdminDashboard() {
  ...
  const [tab,     setTab]     = useState('candidates') // 'candidates' | 'videos'
    ...
    <div className="admin-root">
      ...
      <aside className="admin-sidebar">
        ...
        <nav className="admin-nav">
          ...
          <button
            className={`admin-nav-btn${tab === 'videos' ? ' active' : ''}`}
      ...
      <main className="admin-main">
        {tab === 'candidates' ? <CandidatesPanel /> : <VideosPanel />}
      </main>
    </div>
  )
}
...
function CandidatesPanel() {
  ...
  const toggleApproval = async (candidate) => {
    ...
    if (error) {
      ...
      setCandidates(prev =>
        prev.map(c => c.id === candidate.id ? { ...c, approved: newVal } : c)
      )
      showToast(
        newVal
          ? `✓ ${candidate.name} has been approved.`
          : `${candidate.name} has been revoked.`,
    ...
    <div className="admin-panel">
      ...
      <div className="admin-stats">
        <div className="admin-stat-card">
          ...
          <div>
            <span className="asn">{candidates.length}</span>
            <span className="asl">Total Registered</span>
          </div>
        </div>
        <div className="admin-stat-card admin-stat-card--warning">
          <i className="fas fa-clock" />
          <div>
            <span className="asn">{pending}</span>
            <span className="asl">Pending Approval</span>
          </div>
        </div>
        <div className="admin-stat-card admin-stat-card--success">
          <i className="fas fa-check-circle" />
          <div>
            <span className="asn">{approved}</span>
            <span className="asl">Approved</span>
          </div>
        </div>
      </div>
            ...
            <tbody>
              {filtered.map(c => (
                <tr key={c.id}>
                  ...
                  <td>
                    <span className="course-tag">{c.course}</span>
                  </td>
                  ...
                  <td>
                    <div className="action-btns">
                      <button
                        className={`action-btn ${c.approved ? 'action-btn--revoke' : 'action-btn--approve'}`}
                        onClick={() => toggleApproval(c)}
                        title={c.approved ? 'Revoke access' : 'Approve'}
                      >
      ...
      {confirmDel && (
        <div className="admin-modal-overlay" onClick={() => setConfirmDel(null)}>
          <div className="admin-modal" onClick={e => e.stopPropagation()}>
            ...
            <p>
              Are you sure you want to remove <strong>{confirmDel.name}</strong>?
            ...
            <div className="admin-modal-btns">
              ...
              <button
                ...
                onClick={() => deleteCandidate(confirmDel.id)}
              >
...
function VideosPanel() {
  ...
  const [editVideo, setEditVideo] = useState(null)   // null = add new
  const [toast,     setToast]     = useState(null)
  const [confirmDel, setConfirmDel] = useState(null)
  const [deleting,  setDeleting]  = useState(null)
  ...
  return (
    <div className="admin-panel">
      ...
      <div className="admin-panel-header">
        <div>
          <h1>Videos</h1>
          <p>Manage training video links for all courses</p>
        </div>
        <button className="btn btn-primary" onClick={() => { setEditVideo(null); setShowForm(true) }}>
      ...
      <div className="admin-stats">
        <div className="admin-stat-card">
          ...
          <div><span className="asn">{videos.length}</span><span className="asl">Total Videos</span></div>
        </div>
        <div className="admin-stat-card admin-stat-card--success">
          <i className="fas fa-globe" />
          <div>
            <span className="asn">{videos.filter(v => v.is_public).length}</span>
            <span className="asl">Public</span>
          </div>
        </div>
        <div className="admin-stat-card admin-stat-card--info">
          ...
          <div>
            <span className="asn">{videos.filter(v => !v.is_public).length}</span>
            <span className="asl">Private (course-only)</span>
          </div>
        </div>
      </div>
              ...
              {filtered.map(v => {
                ...
                return (
                  <tr key={v.id}>
                    <td>
                      <div className="video-row-title">
                        ...
                        <div>
                          ...
                          {v.description && (
                            <div className="video-row-desc">{v.description.slice(0, 60)}{v.description.length > 60 ? '…' : ''}</div>
                          )}
                        </div>
                      </div>
                    </td>
                    ...
                    <td>
                      <button
                        className={`visibility-toggle ${v.is_public ? 'vis-public' : 'vis-private'}`}
          ...
          <div className="admin-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="admin-modal-btns">
              ...
              <button
                className="btn admin-btn-danger"
                disabled={deleting === confirmDel.id}
                onClick={() => deleteVideo(confirmDel.id)}
              >
                {deleting === confirmDel.id ? <><span className="spinner" /> Deleting…</> : 'Yes, Delete'}
              </button>
            </div>
...
function VideoFormModal({ video, onClose, onSaved }) {
  ...
  // Auto-derive YT thumbnail when URL is pasted
        ...
        <form onSubmit={handleSubmit}>
          <div className="form-grid-2">
            <div className="form-group">
              ...
              <input
                ...
                placeholder="e.g. Oracle Architecture Overview"
                required
                value={form.title}
          ...
          {ytId && (
            <div className="yt-preview">
              ...
              <div>
                <p className="yt-preview-label">YouTube thumbnail detected</p>
                <p className="yt-preview-id">Video ID: {ytId}</p>
              </div>
            </div>
          )}
            ...
            <label className="visibility-label">
              ...
              <div>
                ...
                <p className="visibility-hint">
                  {form.is_public
                    ? 'Visible on the public /videos page to everyone.'
                    : 'Only accessible to approved students of this course.'}
                </p>
              </div>
            </label>
          </div>
          ...
          <div className="admin-modal-btns">
            ...
            <button type="submit" className="btn btn-primary" disabled={saving}>
              {saving
                ...
                : <><i className={`fas fa-${isEdit ? 'save' : 'plus'}`} /> {isEdit ? 'Save Changes' : 'Add Video'}</>
              }
            </button>
          </div>
        </form>
